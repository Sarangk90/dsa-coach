...
class Session(BaseModel):
    ...
    current_pattern: Optional[str] = Field(default=None, description="Current pattern being learned")
    current_quest: Optional[str] = Field(default=None, description="Current quest being worked on")
    session_type: str = Field(default="general", description="Type: general, learn, practice, review")
...
class Message(BaseModel):
    ...
    tool_name: Optional[str] = Field(default=None, description="Tool name if tool call/result")
    tool_args: Optional[dict] = Field(default=None, description="Tool arguments if tool call")
    created_at: datetime = Field(default_factory=datetime.now)
...
class PatternProgress(BaseModel):
    ...
    quests_completed: int = Field(default=0, description="Quests completed for this pattern")
    quests_total: int = Field(default=0, description="Total essential quests for pattern")
    concepts_understood: list[str] = Field(default_factory=list, description="Understood concepts")
    concepts_total: int = Field(default=0, description="Total concepts in pattern")
    last_practiced: Optional[datetime] = Field(default=None, description="Last practice timestamp")
    next_review: Optional[datetime] = Field(default=None, description="Spaced repetition due date")
...
class QuestCompletion(BaseModel):
    ...
    time_minutes: Optional[int] = Field(default=None, description="Time taken in minutes")
...
class ConceptUnderstanding(BaseModel):
    ...
    diagnosed_at: Optional[datetime] = Field(default=None, description="When diagnosed")
    taught_at: Optional[datetime] = Field(default=None, description="When taught")
...
class Mistake(BaseModel):
    ...
    mistake_type: str = Field(description="Type: off_by_one, edge_case, wrong_pattern, complexity, syntax")
    description: str = Field(description="Description of the mistake")
    lesson_learned: Optional[str] = Field(default=None, description="What was learned from this")
    logged_at: datetime = Field(default_factory=datetime.now)
    recurrence_count: int = Field(default=1, description="How many times this mistake type occurred")
...
class DailyLog(BaseModel):
    ...
    time_spent_mins: int = Field(default=0, description="Time spent learning in minutes")
    patterns_worked: list[str] = Field(default_factory=list, description="Patterns worked on")
...
class Milestone(BaseModel):
    ...
    id: str = Field(description="Unique ID (UUID)")
    user_id: str = Field(default="default")
    milestone_type: str = Field(description="Type: pattern_mastered, streak, no_hints, speed_improvement")
    pattern_id: Optional[str] = Field(default=None, description="Associated pattern if applicable")
    quest_id: Optional[str] = Field(default=None, description="Associated quest if applicable")
...
class TeachingHistory(BaseModel):
    ...
    student_response: str = Field(default="unknown", description="Response: understood, confused, partially")
