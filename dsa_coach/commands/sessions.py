...
def cmd_sessions():
    ...
    try:
        from dsa_coach.ai import delete_conversation, list_saved_conversations
    except ImportError:
    ...
    for i, s in enumerate(saved, 1):
        ...
        _ui.print_styled(
            f"   Messages: {s['messages']} | Saved: {s['saved_at'][:10]}", "dim"
        )

        if s["type"] == "learn":
            _ui.print_styled(f"   Resume: python coach.py learn {s['id']}", "green")
        else:
            _ui.print_styled("   Resume: python coach.py design", "green")
