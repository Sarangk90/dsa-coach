...
def test_start_learning_session_teach_first_uses_existing_intro() -> None:
    ...
    with patch("dsa_coach.ai.learning.get_ai_response", return_value="ok") as mock_resp:
        messages, response = start_learning_session("sliding_window", progress, mode="teach_first")
    ...
    assert called_user_message == LEARNING_SESSION_INTRO.format(pattern="Sliding Window")
...
def test_start_learning_session_diagnose_first_uses_diagnose_intro() -> None:
    ...
    with patch("dsa_coach.ai.learning.get_ai_response", return_value="ok") as mock_resp:
        _messages, _response = start_learning_session("sliding_window", progress, mode="diagnose_first")

    called_user_message = mock_resp.call_args.kwargs["user_message"]
    assert called_user_message == LEARNING_SESSION_DIAGNOSE_INTRO.format(pattern="Sliding Window")
...
def test_interactive_learning_session_with_explicit_mode() -> None:
    ...
    with (
        ...
        patch("dsa_coach.ai.learning.start_learning_session", return_value=([{"role": "user", "content": "x"}], "hi")) as mock_start,
        patch("dsa_coach.ai.ui.print_ai_response"),
        patch("dsa_coach.ai.learning._read_multiline_natural", side_effect=["pause"]),  # Natural multiline reader now
...
def test_interactive_learning_session_resume_with_empty_messages_does_not_crash_and_restarts() -> None:
    from dsa_coach.ai import interactive_learning_session

    saved = {"saved_at": "2025-12-23T00:00:00", "messages": [], "metadata": {"pattern": "sliding_window"}}
    ...
    with (
        ...
        interactive_learning_session("sliding_window", progress={"profile": {}, "pattern_proficiency": {}}, quests=[])
...
def test_interactive_learning_session_autosaves_after_each_exchange() -> None:
    ...
    with (
        patch("dsa_coach.ai.session.load_conversation", return_value=None),
        patch("dsa_coach.ai.learning.start_learning_session", return_value=([{"role": "user", "content": "x"}], "hi")),
        patch("dsa_coach.ai.learning.continue_conversation", return_value=([{"role": "user", "content": "hello"}, {"role": "assistant", "content": "resp"}], "resp")),
        patch("dsa_coach.ai.ui.print_ai_response"),
        patch("dsa_coach.ai.learning._read_multiline_natural", side_effect=["hello", KeyboardInterrupt()]),
        patch(
            "builtins.input",
            side_effect=[
...
def test_multiline_input_sent_as_single_message() -> None:
    ...
    with (
        ...
        patch("dsa_coach.ai.learning.start_learning_session", return_value=([{"role": "user", "content": "x"}], "hi")),
        patch("dsa_coach.ai.ui.print_ai_response"),
        patch("dsa_coach.ai.learning._read_multiline_natural", side_effect=["line1\nline2\nline3", EOFError()]),
        patch(
            "builtins.input",
            side_effect=[
                "t",        # choose teach-first
        ...
        patch("sys.stdin.isatty", return_value=True),
        patch("dsa_coach.ai.session.save_conversation"),
        patch("dsa_coach.ai.learning.continue_conversation", return_value=([], "ok")) as mock_continue,
    ):
        interactive_learning_session("sliding_window", progress={"profile": {}, "pattern_proficiency": {}}, quests=[])
...
def test_editor_command_subl_wait_is_added_when_missing() -> None:
    ...
    with (
        ...
        patch("dsa_coach.ai.learning.start_learning_session", return_value=([{"role": "user", "content": "x"}], "hi")),
        patch("dsa_coach.ai.ui.print_ai_response"),
        patch("dsa_coach.ai.learning._read_multiline_natural", side_effect=[":editor", KeyboardInterrupt()]),
...
def test_editor_alias_e_works() -> None:
    ...
    with (
        ...
        patch("dsa_coach.ai.learning.start_learning_session", return_value=([{"role": "user", "content": "x"}], "hi")),
        patch("dsa_coach.ai.ui.print_ai_response"),
        patch("dsa_coach.ai.learning._read_multiline_natural", side_effect=[":e", KeyboardInterrupt()]),
        patch.dict("os.environ", {"EDITOR": "subl"}, clear=False),
        patch("subprocess.run") as mock_run,
        patch("pathlib.Path.read_text", return_value="hello from editor"),
        patch(
