...
from dsa_coach.tools.obsidian import (
    list_existing_notes as list_notes_tool,
    propose_pattern_note as propose_tool,
    create_pattern_note as create_tool,
    create_problem_note as create_problem_tool,
)
...
def cmd_note(subcommand: Optional[str] = None, *args):
    ...
    if not vault:
        ...
        ui.print_styled("   Example: OBSIDIAN_VAULT_PATH=/path/to/Obsidian/DSA-Prep\n", "dim")
        return
...
def _cmd_note_propose(pattern: Optional[str], ui: UI):
    ...
    # Run proposal tool
    result = asyncio.run(propose_tool(
        pattern=pattern,
...
def _cmd_note_create(pattern: Optional[str], ui: UI):
    ...
    if not pattern_progress:
        ...
        ui.print_styled("   Start learning it first with: python coach.py learn\n", "dim")
        return
    ...
    title = input("Title (e.g., 'Sliding Window'): ").strip() or get_pattern_name(pattern)
    ...
    # Call create tool
    result = asyncio.run(create_tool(
        pattern=pattern,
        title=title,
...
def _cmd_note_update(note_name: Optional[str], ui: UI):
    ...
    result = asyncio.run(update_note_with_insights(
        note_name=note_name,
