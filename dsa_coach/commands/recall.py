...
def cmd_recall():
    ...
    with SyncDatabase() as db:
        ...
        if not due_items:
            ui.print_styled("No items due for review! Great job staying on top of things.", "green")
            return
        ...
        for completion in due_items:
            ...
            quest = next(
                (q for q in all_quests if q.get("problem_id", q.get("id")) == completion.quest_id),
                None
            )
            if not quest:
                continue

            pattern_name = quest.get('pattern_name', quest.get('pattern', 'N/A')).replace('_', ' ').title()
            quest_title = quest.get('problem_name', quest.get('title', completion.quest_id))
            ui.print_styled(f"\n🎯 {quest_title}", "bold")
            ...
            ui.print_styled(f"\nApproach: {hints.get('low', 'Review your solution file.')}", "yellow")
            ...
            completion.next_review_in = _calculate_next_interval(completion.next_review_in, rating)

            db.upsert_quest_completion(completion)

            # Feedback based on rating
            if rating >= 4:
                ui.print_styled(f"Great recall! Next review in {completion.next_review_in} days.", "green")
            elif rating == 3:
                ui.print_styled(f"Decent recall. Next review in {completion.next_review_in} days.", "yellow")
            else:
                ui.print_styled(f"Needs work. Reviewing again in {completion.next_review_in} day(s).", "red")
