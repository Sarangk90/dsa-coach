...
# Maps old quest IDs to new problem IDs
...
def migrate_progress(progress: dict) -> dict:
    ...
    if isinstance(progress.get("completed_quests"), dict):
        ...
        print(
            f"  ✓ Migrated {len(progress['completed_quests'])} completed quests to problems_solved"
        )
    ...
    for _pattern, prof in pattern_prof.items():
    ...
    if "time_spent_hours" not in progress:
        ...
        progress["time_spent_hours"] = {"total": total_mins / 60.0, "by_pattern": {}}
...
def main():
    ...
    print(
        f"  • Total time: {migrated.get('time_spent_hours', {}).get('total', 0):.1f} hours"
    )
