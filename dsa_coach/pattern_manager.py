...
class PatternManager:
    def __init__(self):
        ...
        if isinstance(self.patterns_config, list):
            self.patterns_config = {
                p: {"title": p.replace("_", " ").title()} for p in self.patterns_config
            }
    ...
    def get_pattern_status(self, pattern: str) -> dict[str, Any]:
        ...
        done_essential = [
            qid for qid in essential_ids if qid in self._completed_quest_ids
        ]
        ...
        if next_quest_id:
            next_quest = next(
                (q for q in self.all_quests if q["id"] == next_quest_id), None
            )
        ...
        return {
            ...
            "is_mastered": len(done_essential) == len(essential_ids)
            and len(essential_ids) > 0,
        }
